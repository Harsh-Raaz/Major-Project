...
export default function Notifications() {
  ...
  useEffect(() => {
    const loadNotifications = async () => {
      try {
        const res = await getPatientNotifications(user?.id);
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-blue-700">Stay updated with your appointments and reminders</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border-2 border-dashed border-blue-100 p-8 text-center">
          ...
          <p className="text-sm text-blue-700">You have no notifications at the moment</p>
        </div>
      )}
    </div>
  );
